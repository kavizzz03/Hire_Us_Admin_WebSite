...
<style>
  ...
  h2 { color: #000dff; font-weight: 700; margin-bottom: 1rem; letter-spacing: 1.1px; }
  h4 { color: #222; margin-top: 2rem; margin-bottom: 1rem; font-weight: 600; border-bottom: 3px solid #000dff; display: inline-block; padding-bottom: 0.2rem; }

  .alert { font-weight: 600; border-radius: 8px; background-color: #d1e7dd; color: #0f5132; border-color: #badbcc; box-shadow: 0 0 8px #badbcc; }

  form input, form select { border-radius: 6px; box-shadow: inset 0 0 7px rgb(0 0 0 / 0.05); transition: box-shadow 0.3s ease; }
  form input:focus, form select:focus { box-shadow: 0 0 12px #000dff; outline: none; }

  button.btn { font-weight: 600; transition: background-color 0.3s ease, transform 0.2s ease; border-radius: 8px; }
  button.btn:hover { transform: scale(1.05); }

  table { border-radius: 10px; overflow: hidden; box-shadow: 0 5px 20px rgba(0, 13, 255, 0.1); }
  thead { background-color: #000dff; color: #fff; }
  tbody tr:hover { background-color: #e6f0ff; transition: background-color 0.3s ease; }
  tbody input[type="text"], tbody select { border: 1px solid #ccc; border-radius: 5px; }
  tbody input[type="text"]:focus, tbody select:focus { border-color: #000dff; box-shadow: 0 0 8px #000dff; }

  #chartContainer { margin-top: 3rem; padding: 1.5rem; border-radius: 1rem; background: #f0f4ff; box-shadow: 0 8px 30px rgba(0, 13, 255, 0.1); height: 420px; position: relative; animation: fadeIn 1s ease forwards; }
  #reportSection { margin-top: 3rem; background: #f9faff; border-radius: 12px; padding: 2rem 2.5rem; box-shadow: 0 8px 30px rgba(0, 13, 255, 0.1); animation: fadeIn 1.2s ease forwards; }
  #reportContent h3 { color:#000dff; font-weight:700; margin-bottom:10px; text-align:center; }

  #generateReportBtn, #printReportBtn { border-radius: 8px; font-weight: 600; }
  #printReportBtn { border: 2px solid #000dff; color: #000dff; background: transparent; transition: background-color 0.3s ease, color 0.3s ease; }
  #printReportBtn:hover:not(:disabled) { background-color: #000dff; color: white; }
  #printReportBtn:disabled { opacity: 0.5; cursor: not-allowed; }

  @media (max-width: 768px) { .container { padding: 1.5rem 1.5rem 2rem 1.5rem; } #chartContainer, #reportSection { height: auto; } }

  @keyframes fadeInUp { from {opacity: 0; transform: translateY(30px);} to {opacity: 1; transform: translateY(0);} }
  @keyframes fadeIn { from {opacity: 0;} to {opacity: 1;} }

  footer.footer { margin-top: 4rem; padding: 18px 0; background: #f3f4f6; color: #64748b; font-size: 0.9rem; border-radius: 0 0 1rem 1rem; text-align: center; user-select: none; box-shadow: inset 0 1px 2px rgba(255,255,255,0.7); }
</style>
</head>
<body>
...
<div class="container shadow-lg">
    ...
    <div id="reportSection">
      ...
      <div id="reportContent" class="mt-4" style="display:none;">
        <h3 class="text-center" style="color:#000dff;">Hire Us System</h3>
        <h5 class="mt-2">Report for <span id="reportAdminName"></span></h5>
...
const colors = ['#000dff', '#7c4dff', '#4a60e9', '#e94f37', '#f7b733', '#3fc1c9', '#8e44ad', '#27ae60', '#d35400', '#2980b9'];
...
const loginChart = new Chart(ctx, {
  type: 'bar',
  data: { labels, datasets: [{ label: 'Number of Logins', data, backgroundColor: data.map((_,i) => colors[i % colors.length]), borderWidth: 1, borderColor: '#000dff', borderRadius: 8, maxBarThickness: 70 }] },
  options: {
    scales: { y: { beginAtZero: true, ticks: { stepSize: 1, precision: 0 }, title: { display: true, text: 'Number of Logins' } }, x: { title: { display: true, text: 'Admin Users' } } },
    plugins: { legend: { display: false }, tooltip: { enabled: true, backgroundColor: '#000dff', titleColor: '#fff', bodyColor: '#fff', padding: 8, cornerRadius: 8 } },
    responsive: true,
      ...
      if (data.success) {
        ...
        if (data.logins.length === 0) {
          ...
          data.logins.forEach(login => {
            ...
            td.textContent = new Date(login.login_time).toLocaleString('en-GB', { timeZone: 'Asia/Colombo' });
            tr.appendChild(td);
...
printBtn.addEventListener('click', () => {
  ...
  const header = `<div style="text-align:center; margin-bottom:20px; font-family: 'Poppins', sans-serif;">
                    <h2 style="color:#000dff;">Hire Us System</h2>
                  </div>`;
